...
class FullDuplexManager:
    ...
    def _vad_processor(self):
        """ENHANCED Voice Activity Detection processor"""
        consecutive_speech_frames = 0
        speech_pattern_buffer = deque(maxlen=20)
        ...
        while self.running:
            try:
                ...
                base_threshold = max(self.vad_threshold * 0.8, self.noise_baseline * 2.5)
                
                # Consider recent speech patterns
                if len(self.adaptive_threshold_history) >= 10:
                    recent_avg = np.mean(list(self.adaptive_threshold_history)[-10:])
                    base_threshold = base_threshold * 0.7 + recent_avg * 0.3
                
                adaptive_threshold = base_threshold
                self.adaptive_threshold_history.append(adaptive_threshold)
                
                # MULTI-CRITERIA speech detection
                volume_ok = volume > adaptive_threshold
                peak_ok = peak > adaptive_threshold * 1.2
                
                # Pattern-based detection
                speech_pattern_buffer.append(volume)
                if len(speech_pattern_buffer) >= 10:
                    pattern_trend = np.mean(list(speech_pattern_buffer)[-5:]) > np.mean(list(speech_pattern_buffer)[-10:-5])
                    pattern_ok = pattern_trend and volume > adaptive_threshold * 0.9
                else:
                    pattern_ok = False
                
                # Combined decision
                is_speech = (volume_ok and peak_ok) or pattern_ok
